...
public class Game {
		...
		for (City c : availableCities) {
			if (c.isUnderSiege()) {
				...
				c.setTurnsUnderSiege(c.getTurnsUnderSiege() + 1);
					...
					JOptionPane.showMessageDialog(null, "you should choose between Attack or auto resolve please");
					JFrame f = new JFrame();
					f.setLayout(new FlowLayout());
					f.setSize(800,450);
					f.setVisible(true);
					f.add(BattleView.Autoresolve);
					f.add(BattleView.AttackTheCity);
	...
	public boolean isGameOver() {
		if(player.getControlledCities().size() == availableCities.size() || currentTurnCount > maxTurnCount) {
			...
			f.setVisible(true);
			JLabel A;
			if(player.getControlledCities().size() == availableCities.size()) {
				 A = new JLabel("Congratulations , You Win The Game ");
			}else {
				A = new JLabel("SORRY , You Lost The Game ,Good Luck Next Time");
			}
			f.add(A);
			JButton GameOver = new JButton("GAME OVER");
			GameOver.addActionListener(new helper());
			GameOver.setActionCommand("GAME OVER");
